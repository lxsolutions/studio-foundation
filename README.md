...
| WebGPU shader coverage | 182 of 188 shader modules translate to valid WGSL offline, measured at the engine's real target env (Vulkan 1.1 / SPIR-V 1.3 — the harness previously measured 1.0 and so did not reproduce the engine; see patch 0016). Of the 6 remaining, only `screen_space_reflection_filter` (storage-texture format inference) blocks a feature we want: two are Forward Mobile's subpass tonemap, two are the subgroup variants WebGPU does not select, one is an editor debug gizmo |
| Renderer ceiling | Every WebGPU export ran **Forward Mobile**, which hard-disables SSAO, SSIL, SSR, SDFGI, VoxelGI and volumetric fog regardless of project settings. Patch 0015 unblocks **Forward+** (clustered) — the renderer those effects need, and the better architectural fit for WebGPU, since Mobile's one untranslatable shader is also the engine's only user of subpasses. Translates offline; **not yet hardware-verified**, so `mobile` stays the export default |
