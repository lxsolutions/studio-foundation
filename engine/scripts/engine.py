...
def cmd_versions(lock: dict) -> int:
    ...
    artifact_provenance = lock.get("artifacts", {}).get(
        "export_templates_provenance", {}
    )
    if artifact_provenance.get("patch_through"):
        print(
            "accepted build  : through patch "
            f"{artifact_provenance['patch_through']:04d} "
            f"({artifact_provenance.get('renderer', 'renderer unspecified')})"
        )
    published = lock.get("releases", {}).get("godot_4_7_1_webgpu_p0014", {})
    if published.get("tag"):
        published_records = sum(
            isinstance(published.get(key), dict) for key in TEMPLATE_ARTIFACTS
        )
        print(
            f"published      : {published['tag']} through patch "
            f"{published.get('patch_through', 0):04d} "
            f"({published_records} template(s))"
        )
    forward_plus = lock.get("development", {}).get("webgpu_forward_plus", {})
    if forward_plus.get("patch_through"):
        render_state = (
            "frame rendered" if forward_plus.get("rendered_frame") else "no frame"
        )
        print(
            f"forward+ dev   : patch {forward_plus['patch_through']:04d}, "
            f"{forward_plus.get('gpu_validation_errors', '?')} validation error(s), "
            f"{render_state}"
        )
    return 0
...
def record_artifacts(
    *,
    ...
    lines = [
        ...
        "# Locally accepted build pair generated by engine-record-artifacts after build validation.",
        "# Public release identity is recorded separately under [releases.*].",
    ]
