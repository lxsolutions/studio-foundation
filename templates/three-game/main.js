...
function panelCanvas(size = 512, { cells = 6, grime = 0.4, bolts = true, seed = 5 } = {}) {
    const c = document.createElement('canvas');
    c.width = c.height = size;
    const ctx = c.getContext('2d');
...
function toNormal(canvas, strength = 2.2, repeat = 1) {
 ...
 */
function flute(geo, flutes = 20, depth = 0.035) {
...
if (!hudEl)
    throw new Error('#hud missing from index.html');
