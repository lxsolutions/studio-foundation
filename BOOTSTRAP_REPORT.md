...
It is not a product roadmap. The numbers it, the README, the bforge README and
the GitHub repository description state publicly are enforced against the
pinned artifacts by `tools/ci/check_claims.py` (rules: `docs/claims.toml`), so
none of them can quietly drift from the code again — this file itself did,
once, by three tests, while claiming it could not.
...
| bforge tests | 228 test methods in `tools/bforge/tests` — 178 in suites that boot a real Blender daemon, the rest pure schema/protocol/compiler units; the public CI bench job runs the full suite, not a subset |
...
| Prose/artifact consistency | `tools/ci/check_claims.py` derives op, namespace, test, and patch counts from the pinned artifacts and fails the hosted policy job when a documented surface disagrees — the README, the bforge README, this report, and the GitHub repository description (fetched live; a disagreement fails, an unreachable API is a notice unless `CLAIMS_REQUIRE_REMOTE=1`); absolute-exclusivity claims ("only public …") are rejected outright |
